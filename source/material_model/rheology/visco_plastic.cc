  ...
  namespace MaterialModel
  {
    ...
    namespace Rheology
    {
                                       ...
                                       const std::vector<unsigned int> &n_phases_per_composition) const
      {
        ...
        // Set up additional inputs for elasticity
        const MaterialModel::ElasticInputs<dim> *elastic_inputs =
          in.template get_additional_input<MaterialModel::ElasticInputs<dim> >();

        const bool enable_elasticity = this->get_parameters().enable_elasticity;
        Assert(!(enable_elasticity && elastic_inputs == nullptr), ExcInternalError());

        double dte = numbers::signaling_nan<double>(), 
               mu  = numbers::signaling_nan<double>();
        if (enable_elasticity)
        {
          dte = this->get_elasticity_handler().get_initial_time_step();
          mu  = MaterialUtilities::average_value (volume_fractions, 
                                                  this->get_elasticity_handler().get_elastic_shear_moduli(), 
                                                  viscosity_averaging);
        }

        // If elastic behavior is enabled, then the effects of elasticity should be excluded
        // from strain rate.
        SymmetricTensor<2,dim> effective_strain_rate = in.strain_rate[i];
        if (enable_elasticity)
        {
          Tensor<2,dim> vorticity = 0.5 * (elastic_inputs->velocity_gradients[i] -
                                           transpose(elastic_inputs->velocity_gradients[i]));
          SymmetricTensor<2,dim> rotation_term = symmetrize(- vorticity * elastic_inputs->stress_values[i]
                                                            + elastic_inputs->stress_values[i] * vorticity);

          effective_strain_rate += (elastic_inputs->old_stress_values[i] / dte
                                    - elastic_inputs->convection_terms[i] - rotation_term)
                                   / (2.0 * mu);
        }

        const double edot_ii = ((this->get_timestep_number() == 0 &&
                                 effective_strain_rate.norm() <= std::numeric_limits<double>::min())
                                ?
                                ref_strain_rate
                                :
                                std::max(std::sqrt(std::fabs(second_invariant(deviator(effective_strain_rate)))),
                                         min_strain_rate));
        ...
        const double temperature_for_viscosity = in.temperature[i] + 
                                                 adiabatic_temperature_gradient_for_viscosity * in.pressure[i];
        ...
        for (unsigned int j = 0; j < volume_fractions.size(); ++j)
        {
          ...
          /////////////////////////////////////////////////////////////////////
          // Step 2: elastic behavior
          /////////////////////////////////////////////////////////////////////
          if (enable_elasticity)
          {
            const double mu_dte = mu * dte;
            viscosity_pre_yield = viscosity_pre_yield * mu_dte / (viscosity_pre_yield + mu_dte);
          }

          /////////////////////////////////////////////////////////////////////
          // Step 3: plastic behavior
          /////////////////////////////////////////////////////////////////////

          // Step 3a: strain weakening
          ...
          switch (yield_mechanism)
          {
            ...
            case drucker_prager:
            {
              ...
              if (2. * viscosity_pre_yield * edot_ii >= yield_stress)
              {
      ...
      {
        return strain_rheology.get_strain_composition_mask();
      }
      ...
      ViscoPlastic<dim>::declare_parameters (ParameterHandler &prm)
      {
        ...
        // Reference and miminum/maximum values
      ...
      create_elastic_inputs (MaterialModel::MaterialModelInputs<dim> &in) const
      {
        if (this->get_parameters().enable_elasticity &&
            ...
            in.template get_additional_input<ElasticInputs<dim> >() == nullptr)
        {
          const unsigned int n_points = in.strain_rate.size();
          in.additional_inputs.push_back (
            std::make_unique<MaterialModel::ElasticInputs<dim> >(n_points));
        }
      }

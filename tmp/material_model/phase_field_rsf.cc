...
namespace aspect
{
  namespace MaterialModel
  {
    ...
    void PhaseFieldRSF<dim>::initialize()
    {
      ...
      AssertThrow(this->get_parameters().reconstruct_faults == true,
                  ExcMessage("The phase field RSF model requires the slip rate to calculate the "
                             "friction coefficient. Please set <Formulation/Reconstruct faults from phase field> to true."));
      ...
      AssertThrow(this->get_parameters().nonlinear_solver
                  ...
                  || this->get_parameters().use_implicit_constitutive_model == true,

...
interface.  A solver-local simulator helper named
...
reference for its lifetime.  It owns the particle/Q1 surface assembly,
...
factorized $K_V$ available.  The coupled solver owns the helper for the
duration of its solve.  It is not a persistent \texttt{Simulator} member and
introduces no checkpoint or additional simulator lifecycle hook.
...
actions.  A fault-slip increment changes stress by
\begin{equation}
 \delta\boldsymbol\tau
...
using a solver-local surface helper and its Stage-F inverse operation.  It does
not own constitutive state or connect nonlinear commit/rollback.

...
For the normalization projection, three-point Gauss quadrature may be iterated
over a configured number of equal panels per surface element to resolve
bulk-cell-scale tangential variation. This changes only the approximation of
$b_i=\int_\Gamma N_i(s)I(s)\,ds$; the Q1 mass matrix remains exact.
Normal profiles, profile-uniform material mixtures, and any boundary-completion
data use the same refined surface origins. The rule is fixed per run and profile
IDs are segment/panel/Gauss-point ordered. All mechanical equations and
derivatives consume the resulting single projected $I_h$ field.

No rank shall gather the complete bulk FE field or a complete bulk-node point cloud.

The opt-in cell-interval normalization backend specified in
\texttt{bp3/stage\_K5\_cell\_profile\_addendum.md} may instead cache geometric
ray/cell intersections and integrate current Q1 phase values on bulk-cell
owners, reducing integrals by profile ID. The remote-point backend remains the
default and numerical validation reference. This exception is limited to the
supported affine Box geometry and preserves the profile-uniform mixture,
quadrature/tail coefficients, full physical-box truncation and consistent
surface projection. Phase changes invalidate values but not cell traversals.

\subsection{Normalization-profile material mixture}
For the authorized bounded K5 bottom-completion diagnostic only, augment a
physical profile integral by its missing outside-bottom Q1 continuation before
projection: $b_i=\sum_q w_q N_i(s_q)(I_q^{\rm box}+I_q^{\rm outside})$,
with unchanged $M_{ij}=\sum_q w_q N_i(s_q)N_j(s_q)$ and $M I_h=b$.
Outside data contribute no mechanical domain or forces. The diagnostic is
fresh-start, frozen-profile, mature uniform sliding only; the ordinary
physical-boundary truncation policy remains unchanged. A clipped physical
column is not renormalized to unit integrated localization.

For the qualified opt-in K5 BP3 through-boundary treatment, also continue the bulk
crack-strain source into the in-box wedge beyond the straight bottom tip.
Do not reapply the normal-width cutoff inside this wedge: physical Q1 phase
can remain positive beyond it. Outside the wedge admission is unchanged.
Use physical FE $\phi_h$ and
constant endpoint $I_h,V$ and surface mixture. Already admitted sources are
unchanged. The same source enters Stokes assembly and particle Maxwell-history
updates, but does not admit new particles to surface quadrature or modify
surface topology/ownership. This frozen mature BP3 exception requires prescribed
bottom $V_p$ and immutable completion data for the exact mesh, phase and fault.
It is not a general continuation rule for internal fault tips. The paired
normalization/source treatment is selected by the benchmark parameter
\texttt{Bottom normalization completion file}; legacy diagnostic switches
retain the earlier denominator-only comparison. The same completion input is
reattached before constitutive preparation after restart; derived caches are
rebuilt. No new history update or outside mechanical domain is introduced.

The authorized short top-boundary experiment applies the analogous paired
completion and in-box source continuation at the opposite endpoint, restricted
to frozen mature uniform prescribed slip. Its coordinate is the last segment
at $\xi=1$, with current endpoint fields. This experiment does not revise
surface particle admission. Free-endpoint adoption requires a separate check
of the continued bulk virtual work and its surface traction/$G$ contribution;
prescribed-slip success alone is insufficient.

The subsequent authorized K5 work-measure mode changes mechanical surface
weighting throughout one straight, frozen mature 2-D fault, retaining the
volume-weighted mode as default. With the actual bulk-source extended basis
$b_i=N_i(\widetilde s(x))$, each locally owned physical Stokes QP contributes
exactly once:
\[
 R_i=\sum_q J_q\chi_q b_i(q)
 [\tau_{\rm bg}+\Delta\tau:S-\mu(V_\Gamma,\Theta_{k-1})
 (\sigma_{\rm bg}+\Delta p-\Delta\tau:N)-\eta^d V_\Gamma].
\]
Use exactly the bulk FE phase, completed $I_h$, bulk material coefficients,
and frozen working FE Maxwell history at that QP. Committed surface state
uses the existing interpolation and constant endpoint extension. $C=0$ in
this mode. Every traction term, $K=-R_V$, $G=R_{(u,p)}$, and the mechanical
mass matrix share $J_q\chi_q$; no intact contribution or overlapping wedge
pass is added. Generic particle-property projection is unchanged.
The shear portion of $G$ satisfies the discrete work identity with $B$ on
homogeneous bulk directions; the full $G$ retains normal/friction/pressure
terms and is not $B^T$.
In particular, with frozen $\chi$, bulk Maxwell coefficient $\kappa_b$, and
committed surface state, the common-QP derivatives are
\[
 K_{ij}=\sum_q J_q\chi_q b_i b_j
 [2\kappa_b\chi_q S:S+\sigma_n\mu_V+\eta^d],\qquad
 (G\delta x)_i=\sum_q J_q\chi_q b_i
 [2\kappa_b(S+\mu N):\delta\dot\epsilon-\mu\delta p].
\]
Here $S:N=0$ removes the direct normal-stress $V$ derivative; bulk-mediated
normal feedback remains. Background tractions are fixed constants under
these derivatives. With $\delta\dot\epsilon$ from a homogeneously constrained
bulk test vector, $\delta V^T G_{\rm shear}\delta x=\delta u^T B\delta V$.
For the explicitly authorized initialization-only prescribed-normal control,
replace the normal traction in the friction residual and $K$ by
$\sigma_{\rm bg}+p_{\rm ad}$. Its $G$ contains only
$\sum_q J_q\chi_q b_i\,2\kappa_b S:\delta\dot\epsilon$.
In the BP5 control $p_{\rm ad}=0$ and $\sigma_{\rm bg}=50$ MPa; the independently
solved mechanical $\Delta p-\Delta\tau:N$ is exported separately, not added
to friction. This diagnostic does not change the default pressure mode.
In 2-D, mechanical rows now have units Pa*m and mass units m; traction-scaled
rows and the consistent RMS norm remain Pa. Preserve existing nonlinear
accuracy, active-set signs, merit conventions and solver thresholds.
The qualification uses fixed previously initialized background data without
recalibration and a fresh noncommitting free-endpoint solve, not a reinterpreted
checkpoint. See the K5 work-measure task and qualification report for its
bounded deployment and verification status.

The authorized subsequent committing BP3 replay retains this work measure
and both boundary treatments, with unchanged mature-friction history updates.
It is explicitly selected in the benchmark, starts fresh with the existing
fixed initial background, and is not a reinterpretation of a legacy
checkpoint. Native work-weighted output and legacy parent/domain FE-stress
observations are distinct measures; the latter do not enter mechanics.
Accepted bulk diagnostics use the frozen working FE old history, not the
newly committed particle array. Initial stress and state retention, followed
by the first physical history update, must be checked before continuing.

The separately authorized frozen BP3 free-trace diagnostic permits one
independent free-side rate at the 40-km prescribed/free junction. The deep
trace remains exactly prescribed. Eliminate that constant trace through the
other prescribed endpoint of the adjacent deep segment, using mechanical
weights $(1,0)$ there, while retaining geometric Q1 interpolation of every
history and material property. The same mechanical basis shall enter source,
$B$, residual, $K$, $G$ and norms, with no extra or omitted physical measure.
This explicitly selected noncommitting experiment must roll back before
publication; it does not revise the default continuous-Q1 representation.
See \texttt{bp3/stage\_K5\_free\_trace\_addendum.md} for the bounded scope.
The subsequent authorized seven-step committing experiment additionally uses
the deep node's independent state and slip on the adjacent prescribed segment;
the free trace retains its own nodal histories. Both evolve once per accepted
step, with lagged state in mechanics. It is separately selected and bounded by
\texttt{bp3/stage\_K5\_trace\_replay\_addendum.md}.
The later authorized fully frictional comparison instead removes all deep
prescribed rates, including the bottom endpoint, and retains continuous Q1
fields. Both continued boundary sources use their current free endpoint
rates in the same work equation. See
\texttt{bp3/stage\_K5\_fully\_frictional\_addendum.md} for the seven-step scope.

Following K5 consolidation, the benchmark explicitly selects the successful
fully frictional research configuration; prescribed deep slip remains its
reference/default. The preceding alternative-trace and candidate-state
experiments are archived qualifications, not maintained runtime alternatives.
The current core retains continuous Q1 kinematics and the split committed-state
mechanical equation above. See the K5 cleanup report; no global endpoint or
mature-fault default is changed.
...
\subsection{Explicit mature frictional specialization}

The opt-in parameter \texttt{Fault constitutive mode = mature frictional}
specializes a pre-existing, permanently fixed reconstructed fault. The default
\texttt{cohesive} law above is unchanged. A frozen phase field alone does not
select maturity; no damage-to-mature transition is defined. The mode applies
throughout the selected problem's faults, including prescribed-rate vertices.
Require \texttt{Evolve phase field = false}; initialize the prescribed phase
without a phase equilibrium solve, then retain it. Reject changing fault
geometry and incompatible cohesive/mature checkpoint histories.

The reduced equations are
\[
 C=0,\qquad \upsilon=\frac{h}{I_h}V,\qquad
 F=\tau_{\rm bg}^{\rm eff}+\Delta\boldsymbol\tau:\boldsymbol S
 -\mu(V,\widehat\Theta)
 (\sigma_{n,\rm bg}+\Delta p-\Delta\boldsymbol\tau:\boldsymbol N)-\eta^d V,
\]
\[
 -F_V=2\kappa_{\rm bulk}\chi\boldsymbol S:\boldsymbol S
       +\sigma_n\mu_V+\eta^d,\qquad \chi=h/I_h.
\]
Retain bulk Maxwell stress, B/G, true normal-stress feedback and the existing
nodal split aging update. The direct normal derivative remains zero since
$S:N=0$. Only the cohesive force and $\kappa_\Gamma/I_h$ tangent are removed.
No shared shear modulus or localization normalization is altered.

There is no recoverable cohesive energy: $W_{\rm coh}=0$. The cohesive
driving candidate is zero, hence initialized $H_k=\max(H_{k-1},0)=H_0$
remains inert profile metadata, not evolving physical cohesive work. Phase
and gradient energies are constant. Bulk elastic storage/Maxwell dissipation
remain, as do frictional power $\mu\sigma_n V$ and radiation loss $\eta^dV^2$.
Signed tensile normal traction does not imply positive frictional dissipation.
The fixed background supplies prestress work, not cohesive elastic storage.

For the BP3 comparison, preserve the initial mechanical problem by setting
\[
 \tau_{\rm bg}^{\rm eff}(s)=\widehat\tau_{\rm bg,old}(s)-C_\star(s),\qquad
 C_\star(s)=a(s)+b(s)/d(s),
\]
where the fixed Q1 coefficients are $a_i=\beta_0 C_{0,i}$,
$b_i=\kappa_0 V_{0,i}^{\rm accepted}$, $d_i=I_{0,i}$ from the captured
initial diagnostic. Interpolate coefficients before dividing, rather than
projecting the nonlinear resistance into Q1. These coefficients and the
original background are ordinary manager-owned checkpointed data; their
selector is restored by the initializing plugin. They have zero derivatives
with respect to mechanical unknowns and are never updated from later history.
Mature C is identically zero at initialization and after every accepted step;
retain supplied $\Theta_0$, bulk stress and H at timestep zero. No late
cohesive checkpoint conversion is permitted.
...
\sum_q w_{pq}=m_p.
\]
For a fixed axis-aligned two-dimensional Box with one periodic coordinate,
the approved periodic-domain correction uses the global period and translated
image neighbors, with one owner per real parent. Integrate all physical-Box
fragments of its full periodic domain exactly once. Surface fragments use the
existing open-polyline map, not cyclic endpoint identification. CPDI samples
map periodically into actual FE cells using the existing unique-owner rule.
See \texttt{benchmarking/stage\_K3\_periodic\_domain\_addendum.md} for the
bounded geometry and consumer contract.
...
The approved K5 value-reuse extension caches the completed surface $I_h$ field
as transient data. A collective hit requires exact agreement of owned phase
entries, fault coordinates/versions, mesh-lookup validity, degradation parameters,
and projected surface mixtures. The mixture key is unnecessary when all material
degradation laws are identical; friction fields are still projected afresh.
Mesh deformation disables reuse. Parameter parsing and restart invalidate it;
validity is published only after successful integration and final projection.
A hit issues no adaptive or remote FE requests. The full production integral,
including endpoint truncation, and all tolerances remain unchanged.
Identical current/previous local phase and $I_h$ imply a zero history-localization
difference, but do not imply frozen samples for moving particles.
...
The approved BP3 first-event benchmark additionally checkpoints accumulated
slip, its independent history-audit inputs, last accepted step, event observer,
and output schedule. The benchmark restores the background-traction selector
and deep prescribed-rate map from the fixed configuration; it never recalibrates
the checkpointed background. Accepted-solver and completed-checkpoint observers
do not modify constitutive history, timesteps, or solver decisions. Full
filesystem restart equivalence is required before server-run preparation is
considered complete.

For the explicitly selected fully frictional mature work-measure research
fixture, benchmark checkpoint version 4 additionally preserves its original
inert-$H$, fixed-geometry and completed-$I_h$ audit baseline and validates
the kinematic/work-measure selectors. This targets restart of that same
fixture, not conversion of earlier investigation checkpoints. The original
qualification was blocked by cold-$I_h$ bitwise reproducibility. The approved
long-run correction retains the fixed-profile invariant: for fixed mature
geometry, identical current/previous phase and composition-independent
degradation, recompute once and compare to persisted previous-$I_h$ within
the unchanged quadrature/tail budget, then restore those persisted values
exactly before publishing the transient cache key. Invalid or changed inputs
fail validation; evolved histories are never overwritten. Runtime source
continuations, work-measure selection and background-property indices are
reattached before mechanics; stored histories and background coefficients are
never reinitialized. The bounded saved-clock replay and ordinary adaptive
continuation are separate benchmark modes, both checkpointing accepted states.

The separate modified-BP3 long-run mode uses version-5 benchmark archives and
a new selected-field particle layout; old-layout archives are not converted.
It serializes accumulated signed slip, event state, all light/heavy output
references and clocks, and native fault time-series metadata. A heavy output
decision uses $\max_j|\delta_j-\delta_j^{\rm last\ output}|$ only at accepted
states and is shared by native bulk, particle and fault writers. Advance the
reference only after those writers succeed. Initial and gracefully final
accepted states are included; output triggers do not constrain timesteps.
Checkpoint metadata snapshots identify their actual accepted state. Restart
branches preserve prior evidence and restore the selected metadata prefix.
The benchmark's every-accepted-state cumulative-slip table is independent of
the throttled graphical output. It records local arclength and down-dip
coordinates; restart branches retain only the selected accepted prefix.
Its data are not used to reconstruct constitutive history: accumulated slip
is already checkpointed. Output-only property aliases/exclusions never alter
the manager's stored registry or its checkpoint layout.
Selected-field initial-composition refresh reevaluates only strengthening at
current particle positions; it never resets constitutive history. Empty
selection preserves the generic property's previous behavior.
...
per-fault data. The qualified K5 implementation prefers LAPACK GTTRF/GTTRS:
adjacent-pivoting tridiagonal LU, with one reusable factor
per contiguous principal free block. UMFPACK remains the comparison backend;
neither path assumes positive definiteness. Do not add a bespoke condition
estimator or substitute an SPD-only solve. Factorization failure is a numerical failure. Every
...
The approved BP3 stress-change extension permits fixed surface background
tractions stored in a generic two-component manager Q1 property. They do not
enter bulk assembly or Maxwell history. The point residual becomes
\[
 R_\Gamma=\tau_{\rm pre}+\Delta\boldsymbol\tau:\boldsymbol S-C
 -\mu\bigl(\sigma_{n,\rm pre}+\Delta p
                  -\Delta\boldsymbol\tau:\boldsymbol N\bigr)-\eta^d V.
\]
The $\sigma_n\mu'$ term in $K_V$ uses total normal traction; background
coefficients are frozen and contribute no $B/G$ derivative. Diagnostics must
not infer incremental $\tau:N$ by subtracting total normal stress from
incremental pressure without removing its background. No selected background
property recovers the previous formulation. BP3 initializes zero particle
Maxwell stress change, uses zero top/bottom perturbation traction, and keeps
the official lateral velocities and retained initial state. The Airy field
is an optional analytic diagnostic only, not a bulk prestress to equilibrate.
The BP3 plugin initializes its frozen Q1 shear background with the same
consistent mass and domain quadrature as the surface residual:
\[
 M\boldsymbol\tau_{\rm bg}
 =\left[\int_{\mathcal D_{\rm adm}} N_i
 \left(C_{\rm eval,0}+\mu(V_{\rm init},\Theta_0)\sigma_{n,\rm bg}
                   +\eta^d V_{\rm init}\right)\,d\nu\right]_i,
 \qquad C_{\rm eval,0}=\frac{\kappa_\Gamma V_{\rm init}}{I_{h,0}}
                         +\beta_\Gamma C_0.
\]
Here $d\nu$ denotes the existing admitted-domain surface integration measure;
no domain or support is changed. This balances the represented resistance at
zero stress perturbation, not at zero bulk velocity gradient. In particular,
the slip-induced shear of a zero-rate constitutive probe is not incorporated
into the background. The supplied nodal $\Theta_0$ and stored $C_0$ are retained.
The finite-Q1 difference from $\tau_0^{\rm BP3}+\Pi_{Q1} C_{\rm eval,0}$
must be reported. Later cohesive/state evolution does not change this background.

The explicitly selected modified-BP5 steady-sliding benchmark instead supplies
uniform $\Theta_0=D_c/V_{\rm init}$ and uses the same native weak projection
with $C=0$ to initialize spatially varying background shear.  Project chemical
composition before evaluating friction; use the same work measure, Q1 state
interpolation and endpoint continuation as mechanics.  Replace the captured
background and its correction together, leaving the latter identically zero.
This is an initial-data choice, not a change to subsequent split aging or to
the mature frictional equations.  Checkpoint metadata distinguishes this
choice from inverse-state initializations; restart restores the frozen
background and evolved histories without importing or recalibrating initial
data.

The explicitly selected loading-driven BP5 variant instead initializes
$\Theta_{0,i}=(D_c/V_{\rm init})R_{\rm VW}^{1-f_i}$, with $0<R_{\rm VW}\leq1$
and $f_i=(a_i-a_{\rm VW})/(a_{\rm VS}-a_{\rm VW})$ obtained from the projected
surface material mixture. For the arithmetic two-material friction mixture,
$f_i$ is its strengthening fraction. The default $R_{\rm VW}=1$ retains the
steady variant; the selected loading case uses $R_{\rm VW}=0.8$.
Construct prestress using the actual Q1 interpolation of this nodal state in
the same native weak equation. The particle initial-state input has the
corresponding horizontal-depth extension, but the final projected-mixture
nodal state is authoritative. Record the profile version and ratio in restart
identity; restore rather than reconstruct these initial data. Subsequent
split aging and mechanical/history equations are unchanged.
...
The approved K5 performance prototype may assemble the same discrete $B$ and
$G$ as sparse rectangular matrices for one coupled linearization, retaining
their independent quadrature actions for verification. It may use a bounded
few-mode approximation to $K_V-GP_A B$ to correct the existing Stokes
preconditioner $P_A$, without changing the condensed operator or its residual
criteria. Preserve shared-face averaging, homogeneous constraints and physical
pressure conversion; do not identify $G$ with $B^T$.

The bounded K5 GMG prototype changes only the velocity-block preconditioner
inside the assembled block-Schur inverse. It retains fine-level assembled
$A$, the same $B/G/K_V^{-1}$, pressure-block treatment, and outer FGMRES.
Reuse ASPECT's velocity hierarchy and verify the homogeneous constraints and
active-vector numbering; do not apply an additional matrix-free RHS lift.
Test-only synchronous observers may compare the same frozen operator/RHS
before a nonlinear trial is committed. This does not qualify a full
matrix-free fault solver or a coupled multigrid hierarchy.
...
target. Retain both raw and projected residual checks. Normal convergence output
reports the fresh residual and target; detailed diagnostic mode additionally
reports the raw residual, Arnoldi estimate and compatibility components. This numerical
...
At lower-bound contact, construct the absolute candidate value exactly as
$V_{\rm trial,i}=V_{\min}$, using the same local contact fraction as the limiter.
Non-contact entries retain the affine trial. Copy this validated absolute vector
without cancellation; at exact surface endpoints read the corresponding stored
node directly, including when it is at the lower bound. Copy the same vector
unchanged into temporary residual state and accepted manager state; do not
reconstruct it by subtracting and adding the base rate. The active-set tolerance,
fraction formula and acceptance criteria are unchanged.
...
\paragraph{Phase nonlinear roundoff allowance.}
The phase nonlinear residual target is the maximum of the configured relative
tolerance times its initial residual and $8\epsilon_{\rm double}\|S\|_2$,
fixed at entry to each phase solve. With
$P_p=\sum_j|w_j\phi_j|$, $D_{p,d}=\sum_j|\partial_dw_j\,\phi_j|$,
$a_p=\sum_mf_m(|Hg'_m|+|E_m\alpha'|)$ and
$b_p=\sum_mf_m(|Hg''_m|+|E_m\alpha''|)$, assemble
\[
 S_i=\sum_pm_p\left[|w_i|(a_p+b_pP_p)
             +|F_p|\sum_d|\partial_dw_i|D_{p,d}\right],
 \qquad E_m=\frac{G_{c,m}}{c_0\ell}.
\]
$F_p$ is the phase gradient coefficient. Use the same nonnegative Q1
constraint weights and owned-parent MPI ADD assembly as for the residual.
This conservative first-order roundoff estimate includes nodal representation
and interpolation sensitivity; its arithmetic safety factor is not fitted to
a stalled residual. It is not a rigorous bound for arbitrary ill-conditioned
problems. It introduces no physical parameter, leaves the relative target and
iteration budget intact, and cannot accept stagnation without a sufficiently
small residual. Nonfinite targets are failures. The equations, line search,
history update and benchmark physical criteria are unchanged. The diagnostic
evidence is recorded in
\texttt{benchmarking/stage\_K3\_phase\_precision\_audit.md}.
...
The approved BP3 task permits exact retention of resampled prescribed geometry
(without changing profile-derived support), and prescribed positive surface
slip rates on selected nodes. Lift these essential values into the private
Newton base; use zero increments there and the principal free $K_V$ solve
from the first condensed action. Exclude the replaced friction equations from
convergence/merit, preserve all bulk equations, and publish prescribed $V$
only with the existing terminal commit. Lower-bound active-set release cannot
release essential nodes. The benchmark reapplies its boundary configuration
after reconstruction/restart. Defaults retain the existing unconstrained
frictional geometry/kinematic workflow.

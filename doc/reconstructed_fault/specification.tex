...
initial conservative dimensionless undershoot tolerance is $10^{-4}$; a
global minimum below $-10^{-4}$ shall fail with diagnostics giving the raw
minimum, the tolerance, and the sample location. Every raw sample must be
finite. The upper end shall not be clipped: a raw value above one is an

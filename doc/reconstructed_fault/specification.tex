...
For an initially reconstructed pre-existing fault, form at each associated
particle
\[
...
\]
Here \(H\) is the existing prescribed initial
\texttt{crack\_driving\_force} particle property, \(G\) uses the existing
material mixture and shear-modulus averaging rule, and \(g\) is the existing
phase-field degradation function. The same \(10^{-4}\) empirical
bounded-negative guard used by \(I_h\) applies; values above one are not
clipped.

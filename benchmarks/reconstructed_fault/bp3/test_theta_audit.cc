  ...
  for (const double V : {3.0611065791756763e-17,1e-18,1e-19,1e-20,1e-9})
    {
      ...
      const Reference x=Reference(V)*Reference(dt)/Reference(BP3::Dc);
      const Reference exact=Reference(old)*exp(-x)
                            -Reference(BP3::Dc)/Reference(V)*expm1(-x);
      const long double expected=BP3::aging_state_reference(V,old,dt);
      ...
      std::cout<<std::setprecision(17)<<"V="<<V<<", relative_error="<<error
               <<", incorrect_state_rejected=1\n";
      ++tests;
    }
  if (BP3::aging_state_reference(1e-20,old,0.)!=old)

...
namespace aspect
{
  ...
  {
    unsigned int within_step_target()
    {
      const char *early=std::getenv("ASPECT_BP3_EARLY_TRACE_STEP");
      if (!early) return 10;
      AssertThrow(std::string(early)=="2" && std::getenv("ASPECT_FAULT_FREE_TRACE_DIAGNOSTIC")
                  && std::getenv("ASPECT_FAULT_NONCOMMITTING_DIAGNOSTIC")
                  && !std::getenv("ASPECT_FAULT_WITHIN_STEP_STATE"),
                  ExcMessage("Early trace comparison is restricted to frozen mechanics step 2."));
      return 2;
    }
    ...
    void capture_junction_history(const SimulatorAccess<dim> &sim, AffineConstraints<double> &)
    {
      ...
      if ((!within_step && !theta_audit && !work_audit && !std::getenv("ASPECT_BP3_JUNCTION_DIAGNOSTIC") && !std::getenv("ASPECT_FAULT_HISTORY_FE"))
          || sim.get_timestep_number()!=(within_step ? within_step_target() : work_audit ? 0u : theta_audit ? 13u : 12u))
        return;

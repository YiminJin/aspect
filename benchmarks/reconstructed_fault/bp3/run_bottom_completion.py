"""One authorized bottom-completion comparison, 600-s cap, no retry."""
import hashlib
import json
import os
...
def main():
    assert (OUT/'preflight.json').exists() and not (OUT/'run.log').exists()
    (OUT/'clock.csv').write_bytes((BASE/'clock.csv').read_bytes())
    (OUT/'run.prm').write_text(f'''include {BASE}/run.prm
set Output directory = {OUT}
''')
    previous=json.loads((BASE/'provenance.json').read_text())
    ...
    env.update(ASPECT_BP3_TIMESTEP_SEQUENCE=str(OUT/'clock.csv'),
        ASPECT_IH_BOTTOM_COMPLETION_DIAGNOSTIC=str(OUT/'completion.txt'),
    ...
    paths=[HERE/'bp3.cc',HERE/'uniform_sliding.h',Path(__file__),HERE/'build/libbp3.release.so',
           REPO/'build-pf-cpdi/aspect-release',OUT/'run.prm',OUT/'clock.csv',OUT/'completion.txt',
           REPO/'source/material_model/phase_field_fault.cc']
    ...
    assert result.returncode==0,'Preserve failure; no retry or retuning.'
